...
const Header: FunctionComponent<HeaderProps> = () => {
    let { activeSection, setActiveSection, setTimeLastClick } = useActiveSection();
    return (
        <header className='z-[999] relative'>
            <motion.div className="fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.12] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3rem] sm:w-[36rem] sm:rounded-full dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-65"
                initial={{ y: -100, x: "-50%", opacity: 0 }}
                animate={{ y: 0, x: "-50%", opacity: 1 }}
            >
            </motion.div>
            <nav className='flex fixed top-[0.2rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.5rem] sm:h-[initial] sm:py-0'>
